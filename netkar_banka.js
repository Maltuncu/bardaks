/* Bardaks B2B — Net Kâr V2 + Banka/Kasa + Firma Yönetim Overlay (companion)
...
(function(){
  ...
  function injectStyles(){
    var css=''
    +'#nb-fab{position:fixed;right:18px;bottom:80px;z-index:9000;background:#16a34a;color:#fff;border:none;border-radius:30px;padding:12px 18px;font-size:14px;font-weight:700;box-shadow:0 4px 14px rgba(0,0,0,.35);cursor:pointer}'
...
(function(){
  ...
  function reloadCore(){
    try{ if(typeof loadCoreData!=='function') return;
      loadCoreData().then(function(){ try{ if(typeof CURRENT==='undefined'||!CURRENT||typeof showPage!=='function') return;
        var a=document.querySelector('.nav-item.active'); var pid=a?a.getAttribute('data-page'):(CURRENT.rol==='imalathane'?'uretim':'panel');
        showPage(pid); }catch(e){} }); }catch(e){} }
  ...
  hookAuth();
  ready(function(){
    try{ var le=localStorage.getItem('bardaks_last_email'); var ei=$('emailInput');
      if(le&&ei&&!ei.value){ei.value=le;var pi=$('passInput');if(pi) setTimeout(function(){try{pi.focus();}catch(e){}},80);} }catch(e){}
  ...
  async function forgot(){
    ...
    if(!email){if(err){err.style.color='#c0392b';err.textContent='Once e-postani yaz';}return;}
    if(err){err.style.color='#6b7b8f';err.textContent='Gonderiliyor...';}
    try{ var r=await sb.auth.resetPasswordForEmail(email,{redirectTo:location.origin+location.pathname});
      if(r.error)throw r.error; if(err){err.style.color='#2e8b57';err.textContent=email+' adresine link gonderildi.';} }
  ...
  function showRecovery(){
    ...
    ov.innerHTML='<div style="background:#fff;border:1px solid #e3d8ba;border-radius:12px;padding:28px;width:100%;max-width:360px;box-shadow:0 4px 20px rgba(30,91,168,.12)">'
      ...
      +'<div style="font-size:13px;color:#6b7b8f;margin-bottom:14px">Yeni bir sifre gir.</div>'
    ...
    document.body.appendChild(ov);$('nbSave').addEventListener('click',saveNewPass);
  ...
  async function saveNewPass(){
    ...
    try{ var r=await sb.auth.updateUser({password:p1}); if(r.error)throw r.error;
      err.style.color='#2e8b57';err.textContent='Sifre guncellendi...';
...
(function(){
  ...
  waitApp(function(){
    window.ysLoadUrunler = function(){
      ...
      window.ysSatirlar=fiyatlar.map(function(p){return {urun_id:p.urun_id,ad:p.urun_ad,fiyat:p.birim_fiyat_kdv_haric,kdv:p.kdv_orani,adet:0};}); // [P0] _manual katalog-disi kaldirildi
      ysRenderSatirlar(); };
    window.ysRenderSatirlar = function(){
      var c=document.getElementById('ysUrunler'); if(!c) return; var rows=window.ysSatirlar;
      c.innerHTML='<label style="display:block">Ürünler <span style="font-size:11px;color:var(--text-dim);font-weight:400">(firma kataloğu)</span></label>'
        +(rows.length?rows.map(function(s,i){return '<div class="flex-between" style="padding:7px 0;border-bottom:1px solid var(--border)"><div style="flex:1"><div style="font-size:14px">'+(s.ad||'?')+(s._manual?' <span style="font-size:10px;color:var(--accent)">yeni</span>':'')+'</div><div style="font-size:12px;color:var(--text-dim)">'+(typeof TL==='function'?TL(s.fiyat):s.fiyat)+'/br (KDV hariç) · KDV %'+(s.kdv||1)+'</div></div><input type="number" min="0" style="width:66px;text-align:center" value="'+(s.adet||0)+'" oninput="ysSatirlar['+i+'].adet=Number(this.value)||0;ysHesap()"><button onclick="ysSilSatir('+i+')" style="margin-left:4px;background:none;border:none;color:#e05c5c;font-size:18px;cursor:pointer;padding:2px 6px">✕</button></div>';}).join(''):'<div style="padding:12px;color:var(--text-dim);text-align:center">Firma kataloğu boş — Firma 360 fiyat akışından tanımla.</div>');
      ysHesap(); };
    window.ysSilSatir = function(i){ window.ysSatirlar.splice(i,1); ysRenderSatirlar(); };
    window.ysEklePanel = function(){ return; /* [P0] katalog-disi urun ekleme kaldirildi */
      if(document.getElementById('ysAddPnl')) return;
      var urunOpts=(typeof URUNLER!=='undefined'?URUNLER:[]).filter(function(u){return u.aktif!==false;}).map(function(u){return '<option value="'+u.id+'" data-ad="'+u.ad+'" data-fiyat="'+(u.varsayilan_fiyat||0)+'" data-kdv="'+(u.kdv_orani||1)+'">'+u.ad+'</option>';}).join('');
      var h='<div id="ysAddPnl" style="background:var(--surface2,#1f232c);border:1px solid var(--border);border-radius:10px;padding:12px;margin-top:10px"><div style="font-weight:700;font-size:14px;margin-bottom:8px">Ürün Ekle</div><select id="ysKatSec" style="width:100%;padding:9px;border-radius:8px;border:1px solid var(--border);background:var(--surface);color:var(--text);font-size:14px;margin-bottom:6px" onchange="ysKatDoldur()"><option value="">— Katalogdan seç —</option>'+urunOpts+'</select><input id="ysYeniAd" readonly placeholder="Katalogdan seçilince dolar" style="width:100%;padding:9px;border-radius:8px;border:1px solid var(--border);background:var(--surface);color:var(--text-dim);font-size:14px;margin-bottom:4px;box-sizing:border-box"><div style="display:flex;gap:6px"><input id="ysYeniFiyat" type="number" step="0.01" placeholder="Birim fiyat (KDV hariç)" style="flex:1;padding:9px;border-radius:8px;border:1px solid var(--border);background:var(--surface);color:var(--text);font-size:14px;box-sizing:border-box"><input id="ysYeniKdv" type="number" value="1" placeholder="KDV %" style="width:60px;padding:9px;border-radius:8px;border:1px solid var(--border);background:var(--surface);color:var(--text);font-size:14px;box-sizing:border-box"></div><div style="display:flex;gap:6px;margin-top:8px"><button class="btn btn-sm" onclick="ysEkleOnayla()" style="flex:1">Ekle</button><button class="btn btn-sec btn-sm" onclick="document.getElementById(\'ysAddPnl\').remove()" style="flex:1">İptal</button></div></div>';
      var c=document.getElementById('ysUrunler'); if(c) c.insertAdjacentHTML('beforeend',h); };
    window.ysKatDoldur = function(){
      var sel=document.getElementById('ysKatSec'); if(!sel||!sel.value) return; var o=sel.options[sel.selectedIndex];
      var ai=document.getElementById('ysYeniAd'),fi=document.getElementById('ysYeniFiyat'),ki=document.getElementById('ysYeniKdv');
      if(ai)ai.value=o.getAttribute('data-ad')||''; if(fi)fi.value=o.getAttribute('data-fiyat')||''; if(ki)ki.value=o.getAttribute('data-kdv')||'1'; };
    window.ysEkleOnayla = function(){ alert('Katalog disi urun ekleme kapatildi — fiyat Firma 360 akisindan tanimlanir.'); return; /* [P0] */
      var sel=document.getElementById('ysKatSec'); var ad=(document.getElementById('ysYeniAd')||{}).value||'';
      ...
      if(!uid){alert('Yalnız master katalogdan ürün seçilebilir — serbest ürün girişi kapalı.');return;}
      if(!ad.trim()){alert('Ürün adı gir');return;} if(fiyat<=0){alert('Fiyat gir');return;}
